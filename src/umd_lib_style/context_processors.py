...
def app_info(request: HttpRequest) -> dict[str, Any]:
    ...
    if isinstance(footer_links, str):
        # this is a fully-qualified name to a function to run to get the footer links
        # this allows applications to customize the navigation links on a per-request basis
        module_name, function_name = footer_links.rsplit(".", 1)
        module = import_module(module_name)
        function = getattr(module, function_name)
        footer_links = function(request)

    return {
        "application_name": getattr(settings, "APPLICATION_NAME", "App"),
        "application_version": importlib.metadata.version(package_name)
